...
func (client *Client) EventBus() myevtbus.Bus {
...
func (client *Client) doSubscribe(topic string, fn interface{}, serverAddr, serverPath string, subscribeType SubscribeType) error {
	...
	if *reply {
		return client.eventBus.Subscribe(topic, fn)
	}

	return fmt.Errorf("server not reply")
}
