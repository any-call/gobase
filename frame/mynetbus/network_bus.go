...
func NewNetworkBus(address, path string) *NetworkBus {
	bus := new(NetworkBus)
	bus.sharedBus = myevtbus.New()
	bus.Server = NewServer(address, path)
	bus.Client = NewClient(address, path)
	bus.Server.eventBus = bus.sharedBus
	bus.Client.eventBus = bus.sharedBus
...
func (networkBus *NetworkBus) EventBus() myevtbus.Bus {
